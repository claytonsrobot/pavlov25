...
class Plugin:
    ...
    def __init__(self):
        self.friendly_name = 'Color per Group'
        self.name = os.path.basename(__file__).removesuffix('.py')
        self.dict_known_datapoint_keys=None
        
        
    def prepare_color_style(self):
        # assumes curve_objects in the third tier
        cc=0
        for group in self.scene_object.dict_children.values():
            for subgroup in group.dict_children.values():
                for curve_object in subgroup.dict_curve_objects.values():  
