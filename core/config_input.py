...
class ConfigInput:
    ...
    def pull_specific_values_from_json_config_input_object(self,loaded_json):
        ...
        #ff
        pattern_and = self.loaded_json["filter_files_include_and"]
        pattern_or = self.loaded_json["filter_files_include_or"]
        pattern_not = self.loaded_json["filter_files_exclude"]
        group_names = self.loaded_json["group_names"]
        subgroup_names = self.loaded_json["subgroup_names"]


        #filter_files_object.directory = self.loaded_json["data_directory_relative"]
        ...
        self.skiprows=int(self.loaded_json["data_start_idx"])-1   #38 # for CSV and XLSX import
        #self.tiers = dict()
        #self.stack_vector = None
        """

    def clean_imported_json_numerics(self,loaded_json):
        ...
        for key,value in loaded_json.items():
            ...
            if isinstance(value,str):
                if value.isnumeric():
                    try:
                        loaded_json[key] = int(value) 
                    except Exception:# was not an interger
                        loaded_json[key] = float(value)
        return loaded_json
    ...
    def define_and_load_default_config_input(self):
        ...
        self.loaded_json = self.load_json(self.config_input_path)
        self.loaded_json = self.assign_known_json_filenames(self.loaded_json,self.config_input_path,self.config_entry_filepath)
        #self.loaded_json = self.load_json(filepath)
        self.loaded_json = self.clean_imported_json_numerics(self.loaded_json) # if the json is poorly formatted for nums
        ...
        if self.grouping_algorithm == "group-by-text":
            ...
            self.loaded_json_grouping = self.clean_imported_json_numerics(self.loaded_json_grouping)
        ...
        self.determine_data_directory(self.loaded_json,self.script_dir)
        #self.determine_export_directory(self.loaded_json,self.script_dir)
        #self.pull_specific_values_from_json_config_input_object(self.loaded_json)
        if True: # force to show outdated (free simple) gui
            self.loaded_json["filter_files_include_and"] = ""
            self.loaded_json["filter_files_include_or"] = ""
            self.loaded_json["filter_files_exclude"] = ""
            self.loaded_json["filter_files_exclude"] = ""
            self.loaded_json["export_directory"] = ""
        pprint(f"loaded_json = {self.loaded_json}")
        return 
    
    def assign_known_json_filenames(self,loaded_json,config_input_path,config_entry_filepath):
        loaded_json.update({"config_input_path":config_input_path})
        loaded_json.update({"config_entry_filepath":config_entry_filepath})
        return loaded_json
    
    def determine_data_directory(self,loaded_json,script_dir):
        #self.data_directory = (script_dir+loaded_json["data_directory_relative"]).replace("\\","/")
    ...
    #def determine_export_directory(self,loaded_json,script_dir):
    #    self.export_directory = script_dir+loaded_json["export_directory_relative"]
...
if __name__ == "__main__":
    ...
    config_input_object.pull_specific_values_from_json_config_input_object(loaded_json)
